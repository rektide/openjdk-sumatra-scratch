...
public class LayoutFactoryTest extends TestCommon {
    ...
    public static class IP2 {
        ...
        public Integer a() { return a; }
        public Integer[] b() { return b; }
        public static IP2 valueOf(Integer a, Integer[] b) {
    ...
    public static class IP3 {
        ...
        public Integer b() { return b; }
        public static IP3 valueOf(int a, Integer b) {
    ...
    public void testArray() {
        ...
        Layout<Integer[]> ar_layout = lf.array(Integer.class, 12);
        assertEquals("Size of array", 12 * LayoutFactory.IS, ar_layout.byteOrBitSize());
        assertEquals("Align of array", LayoutFactory.IA, ar_layout.byteAlign());

        Location<Integer[]> ar_loc = ar_layout.allocate();
     ...
     * @param ar_loc
     */
    private void testArray_common(Location<Integer[]> ar_loc) {
        for (int i = 0; i < 12; i++) {
            Location<Integer> i_loc = ar_loc.loc(i);
            assertEquals("Location of array element", i_loc.addr(),
                    ar_loc.addr() + i * LayoutFactory.IS);
        }
    ...
    public void testTuple() {
        ...
        assertEquals("Size of tuple", 2 * LayoutFactory.IS, ip1_layout.byteOrBitSize());
        assertEquals("Align of tuple", LayoutFactory.IA, ip1_layout.byteAlign());
    ...
    public void testBitTuple1() {
        ...
        assertEquals("Size of tuple", LayoutFactory.IS, ip1_layout.byteOrBitSize());
        assertEquals("Align of tuple", LayoutFactory.IA, ip1_layout.byteAlign());
    ...
    @Test
    public void testBitTuple1Signed() {
        LayoutFactory lf = new LayoutFactory();
        ...
        Location<IP1> loc = ip1_layout.allocateWithinArray();
        {
        loc.put(new IP1(3,-4));
        IP1 ip = loc.val();
        assertEquals(ip.a, 3);
    ...
    private void testBitTuple_common(LayoutFactory lf, Layout<IP1> ip1_layout) {
        System.out.println(ip1_layout);
        Pair<Location<IP1>> loc = simpleStoreLoad(ip1_layout);
        dumpBytes(loc.a(), lf);
        dumpBytes(loc.b(), lf);
    }
    ...
    public void testBitTuple2() {
        LayoutFactory lf = new LayoutFactory();
        Layout<IP1> ip1_layout = lf.tuple(IP1.class, "a:16", "b:16");
        assertEquals("Size of tuple", LayoutFactory.IS, ip1_layout.byteOrBitSize());
        assertEquals("Align of tuple", LayoutFactory.IA, ip1_layout.byteAlign());
    ...
    public void testBitTuple3() {
        LayoutFactory lf = new LayoutFactory();
        Layout<IP1> ip1_layout = lf.tuple(IP1.class, "a:17", "b:17");
        assertEquals("Size of tuple", 2*LayoutFactory.IS, ip1_layout.byteOrBitSize());
        assertEquals("Align of tuple", LayoutFactory.IA, ip1_layout.byteAlign());
    ...
    public void testBitArray() {
        LayoutFactory lf = new LayoutFactory();
        Layout<Integer> elt = lf.bitfieldLayoutFor(Integer.class, 5);
        ArrayLayout<Integer> ar_layout = lf.array(elt, 12);
        assertEquals("Size of array", 2 * LayoutFactory.IS, ar_layout.byteOrBitSize());
        assertEquals("Align of array", LayoutFactory.IA, ar_layout.byteAlign());

        ArrayLocation<Integer> ar_loc = ar_layout.allocate();
//        for (int i = 0; i < 12; i++) {
//            Location<Integer> i_loc = ar_loc.loc(i);
//            System.out.println(i_loc);
////            assertEquals("Location of array element", i_loc.addr(),
////                    ar_loc.addr() + i * StaticLayoutFactory.IS);
//        }

        for (int i = 0; i < 12; i++) {
            ar_loc.put(i, i);
            Integer[] v = ar_loc.val();
            assertEquals("Value of array element", i, ar_loc.val(i).intValue());
        }
        Integer[] v = ar_loc.val();
        for (int i = 0; i < 12; i++) {
            assertEquals("Value of result", i, v[i].intValue());
    ...
    public void testTupleWithArray() {
        ...
        assertEquals("Size of tuple", 5 * LayoutFactory.IS, ip2_layout.byteOrBitSize());
        assertEquals("Align of tuple", LayoutFactory.IA, ip2_layout.byteAlign());
    ...
    public void testTupleWithPrimitive() {
        ...
        assertEquals("Size of tuple", 2 * LayoutFactory.IS, ip3_layout.byteOrBitSize());
        assertEquals("Align of tuple", LayoutFactory.IA, ip3_layout.byteAlign());
    ...
    public void testBitMatrix() {
        ...
        { // native mem, unsigned
            Layout<Integer> elt = lf.bitfieldLayoutFor(Integer.class, 5);
            Matrix<Integer> m = mf.matrix(elt, 5, 6);
        ...
        { // heap mem, unsigned
            Layout<Integer> elt = lf.bitfieldLayoutFor(Integer.class, 5);
            Matrix<Integer> m = mf.matrixInHeap(elt, 5, 6);
     ...
     * @param m
     */
    private void testBitMatrix_common(LayoutFactory lf, Matrix<Integer> m, int bias) {
        for (int i = 0 ; i < m.nRows(); i++)
            for (int j = 0; j < m.nCols(); j++)
                m.put(i, j, bias + (int) m.nCols()*i+j);

        for (int i = 0 ; i < m.nRows(); i++)
            for (int j = 0; j < m.nCols(); j++)
                assertEquals(new Integer(bias + (int) m.nCols()*i+j), m.val(i, j));
