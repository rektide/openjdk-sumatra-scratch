...
/**
 * A layout for a 1-bit boolean, in an 8-bit container.
 */
public final class Z extends AtomLayout<Boolean> {
    ...
    /**
     * Used for wiping zero/sign bits across boolean bitfields stored in
     * an int-sized value.
     */
    ...
    static byte bool(boolean bv) {
        return (byte) (bv ? 1 : 0);
    }

    static boolean bool(byte v) {
        return v != 0;
    }

    static boolean bool(int v) {
        return (0xFF & v) != 0;
    }

    Z(Class cl, int s, int a) {
        super(s, a, cl);
    }

    // NOT PUBLIC, but visible to Location
    boolean prim(Object base, long l) {
        return bool(getBox(base, l));
    }

    // NOT PUBLIC, but visible to Location
    void putPrim(Object base, long l, boolean v) {
        putBox(base, l, (byte)(v ? 1 : 0));
    }

    // NOT PUBLIC, but visible to Location
    @Override
    Boolean val(Object base, long l) {
        return new Boolean(prim(base, l));
    }

    // NOT PUBLIC, but visible to Location
    @Override
    void put(Object base, long l, Boolean v) {
        putPrim(base, l, v.booleanValue());
    }
    ...
    @Override
    void fillArray(Location arena, Object base, long addr, Boolean[] a) {
        int count = a.length;
        long inc = byteOrBitSize();
        for (int i = 0; i < count; i++) {
            a[i] = Boolean.valueOf(prim(base, addr));
            addr += inc;
        }
    }
    ...
    @Override
    AtomBitLayout<Boolean> bitsWideLE(int b) {
        if (b < 0) { // Signed
            throw new Error("Boolean bitfields have no sign");
        } else { // Unsigned
            if (b > BITS_PER_BOX)
    ...
    @Override
    AtomBitLayout<Boolean> bitsWideBE(int b) {
        if (b < 0) { // Signed
            throw new Error("Boolean bitfields have no sign");
        } else { // Unsigned
            if (b > BITS_PER_BOX)
     ...
     * big/little signed/unsigned boolean bit fields.
     ...
     */
    abstract public static class Bit extends  AtomBitLayout<Boolean> {
        @Override
        ...
        Bit(int s, int mask) {
            super(s, 0, Boolean.class);
            storeMask = mask;
        }

        abstract boolean prim(Object base, long l, long b);
        abstract void putPrim(Object base, long l, long b, boolean v);
        ...
        // NOT PUBLIC, but visible to Location
        @Override
        Boolean val(Object base, long l, long b) {
            return new Boolean(prim(base, l, b));
        }

        // NOT PUBLIC, but visible to Location
        @Override
        void put(Object base, long l, long b, Boolean v) {
            putPrim(base, l, b, v.booleanValue());
        }
        ...
        void fillArray(Location arena, Object base, long addr, Boolean[] a) {
            ...
            long bump = byteOrBitSize();
            for (int i = 0; i < count; i++) {
                a[i] = Boolean.valueOf(prim(base, addr, offset));
                offset += bump;
            }
        }
        ...
        @Override
        final Layout<Boolean> atFixedOffset(int b) {
    ...
    abstract static class BitLE extends  Bit {
        ...
        // NOT PUBLIC, but visible to Location
        @Override
        final void putPrim(Object base, long l, long b, boolean bv) {
            byte v = bool(bv);
    ...
    public final static class BitLEUnsigned extends BitLE {
        ...
        @Override
        final boolean prim(Object base, long l, long b) {
            ...
            if (before_bits_above >= 0) {
                ...
                return bool(container >>> (ADJUST + after_bits_above));
            } else {
                // Should not happen with layouts derived from C data structures
    ...
    public final static class BitBEUnsigned extends BitBE {
        ...
        // NOT PUBLIC, but visible to Location
        @Override
        boolean prim(Object base, long l, long b) {
            ...
            if (ib >= 0) {
                ...
                return bool((container >>> ib) & storeMask);
            } else {
                // Should not happen with layouts derived from C data structures
                // But it handles the array case nicely.
                int slop = BITS_PER_BOX + ib;
                ...
                return bool(storeMask & ((container << ib) | (other_container >>> slop)));
            }
        }
     ...
     */
    abstract public static class BitAt extends Layout<Boolean> {
        ...
        // NOT PUBLIC, but visible to Location
        abstract boolean prim(Object base, long l);

        // NOT PUBLIC, but visible to Location
        final void putPrim(Object base, long l, boolean bv) {
            byte v = bool(bv);
        ...
        // NOT PUBLIC, but visible to Location
        @Override
        final Boolean val(Object base, long l) {
            return new Boolean(prim(base, l));
        }

        // NOT PUBLIC, but visible to Location
        @Override
        final void put(Object base, long l, Boolean v) {
            putPrim(base, l, v.booleanValue());
        }

        @Override
        final void fillArray(Location arena, Object base, long addr, Boolean[] a) {
            throw new UnsupportedOperationException(
    ...
    public static class BitAtSigned extends BitAt {
        ...
        boolean prim(Object base, long l) {
            ...
            // Assume bitfields do not span an int-sized boundary.
            return bool(container >> (bitOffset + bitsAbove));
        }
    }
